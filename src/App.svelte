...
<main class="bg-[url('./assets/104824.webp')]" style="background-size: cover;background-repeat: no-repeat;">
    <div class="font-['poppins text-[#0070ef] h-screen flex flex-col justify-center items-center">
        ...
        <slot />
        {#if vrijeme}
            <div class="flex flex-col justify-center items-start drop-shadow-2xl rounded-lg md:rounded-3xl bg-white px-5 py-5 md:p-14 w-[90%] md:w-fit md:text-xl gap-4 md:gap-8">
                <form class="text-gray-800 mb-3" on:submit|preventDefault={handleSubmit}>
